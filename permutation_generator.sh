...
for ((i = 2; i <= declared_number_of_lines + 1; i++)); do
	...
	./generator_main $current_line |
		while IFS= read -r line; do
			...
			if [[ $operation = "TWO_LINE" ]]; then
				echo "\subsubsection{Two-line notation:}" >>generated_file.tex
				echo "$\bigl(\begin{smallmatrix}" >>generated_file.tex
				echo "$result" >>generated_file.tex
				echo "\end{smallmatrix}\bigr)$" >>generated_file.tex
				echo "" >>generated_file.tex
